...
const EchoManager = createReactClass({
    propTypes: {
    ...
    render() {
        ...
        return <span>
            {Object.keys(EchoAnimationTypes).map(animationType => {
                ...
                return <TransitionGroup key={animationType}>
                    {echoesForType.map((echo, index) => {
                        const {animationId} = echo;
                        console.log(animationId)
                        return <CSSTransition key={animationId} enter={true} exit={false} classNames={animationTransitionName} timeout={{enter: animationDurationMs, exit: 0}}>
                            <Echo
