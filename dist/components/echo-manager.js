...
var _require3 = require('./prop-types'),
    echoPropType = _require3.echoPropType,
    bordersPropType = _require3.bordersPropType,
    boundingBoxPropType = _require3.boundingBoxPropType,
    keyIdPropType = _require3.keyIdPropType;

var zIndexes = require('./z-indexes');

var Echo = createReactClass({
    displayName: 'Echo',
...
var EchoManager = createReactClass({
        ...
        return React.createElement(
            ...
            Object.keys(EchoAnimationTypes).map(function (animationType) {
                ...
                return React.createElement(
                    TransitionGroup,
                    { key: animationType },
                    echoesForType.map(function (echo, index) {
                        var animationId = echo.animationId;

                        console.log(animationId);
                        return React.createElement(
                            CSSTransition,
                            { key: animationId, enter: true, exit: false, classNames: animationTransitionName, timeout: { enter: animationDurationMs, exit: 0 } },
                            React.createElement(Echo, _extends({
                                animationDurationMs: animationDurationMs,
                                onAnimationFinish: function onAnimationFinish() {
                                    _onAnimationFinish(animationId);
                                }
                            }, echo))
                        );
                    })
